...
model = dict(
        ...
        teacher = dict(
            type="VoxelNet",
            pretrained='work_dirs/history/voxelnet/teacher/latest.pth',
            reader=dict(
        ...
        student = dict(
            ...
            backbone=dict(
                type="SpMiddleResNetFHD", num_input_features=5, ds_factor=8, student=False),
            neck=dict(
                type="RPN",
                layer_nums=[5, 5],
                ds_layer_strides=[1, 2],
...
log_level = "INFO"
work_dir = './work_dirs/half/voxelnet/{}/'.format(__file__[__file__.rfind('/') + 1:-3])
load_from = None
resume_from = None
workflow = [('train', 1)]

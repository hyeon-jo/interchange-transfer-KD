...
from det3d.torchie.trainer import save_checkpoint
...
def main():
    ...
    if hasattr(cfg, 'teacher_model'):
        teacher_model = build_detector(cfg.teacher_model)
        model = {'student': model, 'teacher': teacher_model}
    if args.print_model:
        if args.local_rank == 0:
            print(model)
            print(cfg.work_dir+'model0.pth')
            save_checkpoint(model, cfg.work_dir+'model0.pth')
        exit(0)
    summary(teacher_model,(64615, 20, 5))
    raise
    ...
    if isinstance(model, dict):
        model['student'].CLASSES = datasets[0].CLASSES
        model['teacher'].CLASSES = datasets[0].CLASSES
    else:

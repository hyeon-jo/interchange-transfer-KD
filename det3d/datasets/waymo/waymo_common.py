...
def get_obj(path):
    path = os.path.join('/home/hyeon/CenterPoint/', path)
...
def _create_gt_detection(infos, tracking=True):
    ...
    for idx in tqdm(range(len(infos))):
        ...
        annos = get_obj(info['anno_path'])['objects']
    ...
    # Write objects to a file.
    print(count, 'files were proceed')
    print(box_count, 'files has no box')
    print(point_count, 'boxes has no point')
    print(unknown_count, 'boxes has unknown class')
...
def parse_args():
    ...
    parser.add_argument("--path", type=str, default="/home/hyeon/CenterPoint/data/Waymo/tfrecord_training")

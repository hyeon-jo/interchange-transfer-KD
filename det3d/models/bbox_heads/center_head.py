...
class CenterHead(nn.Module):
    ...
    def forward(self, x, shared=True, *kwargs):
        ret_dicts = []
        ...
        # TODO: x is shared conv, remove later
        return ret_dicts, x
        # return ret_dicts
    ...
    def loss(self, example, preds_dicts, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            hm_loss = self.crit(preds_dict['hm'], example['hm'][task_id],
                                example['ind'][task_id], example['mask'][task_id], example['cat'][task_id])
            ...
            ret.update({'loss': loss, 'hm_loss': hm_loss.detach().cpu(), 'loc_loss':loc_loss,
                        'loc_loss_elem': box_loss, 'num_positive': example['mask'][task_id].float().sum()})
    ...
    def post_processing(self, batch_box_preds, batch_hm, test_cfg, post_center_range, task_id):
        ...
        for i in range(batch_size):
            ...
            boxes_for_nms = box_preds[:, [0, 1, 2, 3, 4, 5, -1]]        # 0, 1, 2 = Center [x, y, z]; 3, 4, 5 = Size [l, w, h]; 6 = Rotation
